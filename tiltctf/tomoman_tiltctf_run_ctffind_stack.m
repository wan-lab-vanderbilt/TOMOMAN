...
function tomoman_tiltctf_run_ctffind_stack(tomolist,ps_name,ctffind,diag_name)
...
% Write run file
ctffind_filename = [tomolist.stack_dir,'/tiltctf/run_ctffind.sh'];
fid = fopen(ctffind_filename,'w');
fprintf(fid,'%s\n','ctffind --amplitude-spectrum-input << fin');
fprintf(fid,'%s\n',input_name);
fprintf(fid,'%s\n','no');
fprintf(fid,'%s\n',diag_name);
