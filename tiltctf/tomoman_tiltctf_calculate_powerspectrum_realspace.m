...
%     ss=zeros(256,256);
    for j = 1:n_pos
        ...
        temp_amp = abs(fftshift(fft2(tomoman_tiltctf_prepare_tile(tile,20))));
        ...
        %p_res_amp = sg_fourier_rescale_image(temp_amp,p_scale_factor*fscaling); ORIG
...
%         %diagnostic plots
%         subplot(2,3,1); tom_imagesc(p_ps_stack(:,:,i));title(['sum pos PS Nr ' num2str(j)]); drawnow;
%         subplot(2,3,2); tom_imagesc(temp_amp);title(['unstr PS Nr ' num2str(j)]); drawnow;
%         subplot(2,3,3); tom_imagesc(p_res_amp);title(['pos PS Nr ' num2str(j)]); drawnow;
%         subplot(2,3,4); tom_imagesc(tile);title(['real Nr ' num2str(j)]); drawnow;
%         ss=ss+temp_amp;
%         subplot(2,3,5); tom_imagesc(ss);title(['unstr Nr ' num2str(j)]); drawnow;
%         subplot(2,3,6); tom_imagesc(u_ps_stack(:,:,i));title(['sum unstr PS Nr ' num2str(j)]); drawnow;       
