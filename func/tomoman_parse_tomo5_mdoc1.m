function mdoc_param = tomoman_parse_tomo5_mdoc1(mdoc_name,fields,field_types,subframe_bug)
%% tomoman_parse_mdoc
...
img_start = find(cellfun(@(x) ~isempty(strfind(x,img_start_str)), mdoc{1}));    % Get line numbers for starting strings (CHECK CHECK: possible version issue!!!)
...
if any(taa_idx)
    ...
    % Find first 'Binning' after string
    taa_end = strfind(mdoc{1}{taa_cell_idx}(taa_str_idx+taa_start(1):end),'Binning');
    ...
    for j = 1:n_fields
        ...
        switch fields{j}
            ...
            otherwise
                ...
                % Generate frame names based on Tomo5 convention
                if subframe_bug
                    mdoc_param(i).SubFramePath = tomoman_lookup_tomo5_frame(mdoc_name,mdoc_param(i),i);
                end
