...
if strcmp(mc2.input_format,'eer')
    if ~isempty(tomolist.defects_file)
        ...
        error('You must provide defect map when motion correcting EER!!!');
    end
end
...
if strcmp(mc2.input_format,'eer')
    if ~isempty(mc2.EerSampling)
        ...
        error('You must provide EER sampling when motion correcting EER!!!');
    end
end
...
for i = 1:n_img
    ...
    if strcmp(mc2.input_format,'eer')
        if ~isempty(mc2.FmIntFile)
            ...
            if ~isempty(mc2.EerGrouping)
                ...
                error('You must provide either EER grouping or FmIntFile when motion correcting EER!!!');
            end
        end
    end   
    ...
    mc2_cmd = ['motioncor2 ',in_str,input_names{i},' -OutMrc ',output_names{i},' -LogFile ',output_names{i},'.log ',gain_str,dosefilter_str,eer_str,other_param,splitsum_str];
    system(mc2_cmd);
