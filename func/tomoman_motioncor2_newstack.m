...
for i = 1:n_stacks
    ...
    if process        
        ...
        tomo_str = strrep(tomolist.mdoc_name, '.st.mdoc', '');
        ...
        if mc2.SplitSum == 1
            if ~isempty(a.stack_prefix) && (~strcmp(a.stack_prefix, 'AUTO'))
                stack_names = {[p.prefix,tomo_str,'_ODD.st'],[p.prefix,tomo_str,'_EVN.st']};
            else
            ...
            for j = 1:num_stacks
                ...
                for k = 1:n_tilts
                    ...
                    % Read image
                    img = sg_mrcread([mc2_dir,tomo_str,'_',num2str(k),'_',stack_types{j},'.mrc']);
