function tomoman_imod_batchprocess_generate_tomogram_runscript(t,p)
...
% Open run script
rscript = fopen([t.stack_dir,'/imod_batchprocess/run_IMOD.sh'],'w');
...
%             'source ~/.bashrc\n\n',]);                      % Get proper envionment; i.e. modules        
    ...
    otherwise
            error('only "local" or "p.hpcl67" or "p.hpcl8" are supported queques for p.queue!!!!')
...
fprintf(rscript,[t.stack_dir,'imod_batchprocess/stack_process.sh\n\n']);
...
for i = 1:numel(p.tomo_bin)
    ...
    fprintf(rscript,[p.fcrop_vol,' ',...
                     ...
                     '> ',t.stack_dir,'imod_batchprocess/binning_log.txt 2>&1','\n\n']);
...
fclose(rscript);
system(['chmod +x ',t.stack_dir,'/imod_batchprocess/run_IMOD.sh']);
