function tm_export2warptools_export_tomogram(t,warpdir,imod_stack)
...
if n_alitilts == n_tilts
    ...
    warning(['Stack: ' name ' Something is seriously wrong with tilt angles!! Check whether number of tilts match between cleaned and aligned stack!!'])
end
...
% tlt file
tlt_targetfile = [WarpImodDir,'/',TomoName,'.tlt'];
if exist(xf_sourcefile,'file')
    % invert sign of tilt angles
    tlt = dlmread(tlt_sourcefile);
    dlmwrite(tlt_targetfile, tlt.*-1);
else
