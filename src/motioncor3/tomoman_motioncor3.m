function par = tomoman_motioncor3(root_dir,paramfilename,par)
%% tomoman_motioncor3
% A function to run Shawn Zheng's MotionCor3
%
% Input comes from the tomoman_motioncor3.param file.
%
% WW 08-2025

%%%%% DEBUG
% root_dir = pwd;
% paramfilename = 'tomoman_motioncor3.param';
% par = [];
...
% Parse are-struct
mc3_fields = tm_get_motioncor3_fields();
mc3 = tm_parse_param(mc3_fields,param_cell);
...
dep = tm_get_dependencies(p,'motioncor3',dep);        % MotionCor3
...
if par_proc
    ...
    % Split tomolist
    [tomolist,p.tomolist_name,par] = tm_split_tomolist(tomolist,p.root_dir,p.tomolist_name,par,'motioncor3',mc3);
    if isempty(tomolist)
        par.last_task = 'motioncor3';
        return
    end
    
    % Override GPU settings
    mc3.Gpu = par.task_gpu - 1; % Adjust GPU ID to start at 0
end
...
while all(t <= n_tilts)
    ...
    % Run motioncor2
    tomolist(t) = tm_motioncor3_newstack(tomolist(t),p,dep,mc3,write_list);
