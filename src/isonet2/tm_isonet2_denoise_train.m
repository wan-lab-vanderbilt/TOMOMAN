function tm_isonet2_denoise_train(p, isonet2, isonet2_fields, dep)
%% tm_isonet2_denoise_train
% Run isonet2 denoising training.
%
% WW 02-2026


%% Initialize
disp([p.name,'Preparing tomograms for IsoNet2 Denoising Training!!!1!']);
...
training_star_filename = [isonet2_dir,'denoise_training.star'];
...
script_name = [isonet2_dir,'run_denoise_training.sh'];
log_name = [isonet2_dir,'denoise_training.log'];
...
fprintf(script,['echo "##### Running IsoNet2 Denoise Training #####"','\n\n\n']);

% Write script
fprintf(script,[dep.isonet2,' denoise ',training_star_filename]);
...
for i = 13:size(isonet2_fields,1)
    % Skip empty field
...
% Run file
disp([p.name,'Running IsoNet2 denoise training...']);
system(script_name);
