function tm_isonet2_refine_predict(p, isonet2, dep, par)
%% tm_isonet2_refine_predict
% Run IsoNet2 refinement with a pre-trained network.
%
% WW 02-2026


%% Initialize
disp([p.name,'Preparing tomograms for IsoNet2 Refine Prediction!!!1!']);


% Check directories
isonet2_dir = [p.root_dir,isonet2.isonet2_dir];
training_dir = [isonet2_dir,isonet2.refine_training_subdir];
predict_dir = [isonet2_dir,isonet2.refine_predict_subdir];
...
if sg_check_param(isonet2,'predict_subset_list')
    % Read subset list
...
if ~isempty(par)
    ...
    predict_star_filename = [isonet2_dir,'refine_predict_',num2str(par.task_id),'.star'];
    ...
    % Parse output star name
    predict_star_filename = [isonet2_dir,'refine_predict.star'];
...
if ~isempty(par)
    script_name = [isonet2_dir,'run_refine_predict.sh_',num2str(par.task_id)];
    log_name = [isonet2_dir,'refine_predict.log_',num2str(par.task_id)];
else
    script_name = [isonet2_dir,'run_refine_predict.sh'];
    log_name = [isonet2_dir,'refine_predict.log'];
end
...
fprintf(script,['echo "##### Running IsoNet2 refine Prediction #####"','\n\n\n']);
...
% Run file
disp([p.name,'Running IsoNet2 refine prediction...']);
system(script_name);
...
if ~isempty(par)
    ...
    % Write completed com
    system(['touch ',par.comm_dir,'tomoman_isonet2_refine_predict_',num2str(par.task_id)]);
    
    % Run process or wait
    if par.task_id == 1
        % Wait for parallel jobs
        disp([par.name,'Waiting for all tasks to finish...']);
        tm_wait_for_them(par.comm_dir,'tomoman_isonet2_refine_predict',par.n_tasks,5);
        
        % Assemble star files
        disp([par.name,'All parallel star files written!!! Assembling full star file...']);
        tm_isonet2_update_star(input_star,[isonet2_dir,'refine_predict'],input_star_name,true,size(job_array,1)); 
        ...
        system(['touch ',par.comm_dir,'tomoman_isonet2_refine_predict_star']);
    else
        % Wait for sorting to finish
        disp([p.name,'Waiting for full star files...']);
        tm_wait_for_it(par.comm_dir,'tomoman_isonet2_refine_predict_star',5);  
    end
