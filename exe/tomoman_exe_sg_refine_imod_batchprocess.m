...
p.subtomo_dir = '/fs/pool/pool-plitzko/Sagar/Projects/invitro_t20s/tfs_cfeg/subtomo_test/bin4/';
...
p.queue = 'p.hpcl67'; % Queue: "local" or "p.hpcl67" or "p.hpcl8"!

% Outputs
script_name = 'sg_refine_imod_ctfphaseflip';    % Root name of output scripts. One is written for each computer.
...
% Bin tomogram
p.justbin = 1 ;       % 1 = just run the binning operation. 0 = otherwise.
...
p.main_dir = [p.root_dir 'bin2_sg_refine_ctfphaseflip/'];  % Destination of first tomogram (MAKE SURE IT"S THE RIGHT BINNING)
p.bin_dir = {[p.root_dir 'bin4_sg_refine_ctfphaseflip/'],[p.root_dir 'bin8_sg_refine_ctfphaseflip/']};   % Destination of binned tomograms. For multiple binnings, supply as cell array.
